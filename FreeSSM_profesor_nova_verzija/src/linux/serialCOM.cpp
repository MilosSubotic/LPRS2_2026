...
bool serialCOM::GetPortSettings(double *baudrate, unsigned short *databits, char *parity, float *stopbits)
{
	...
	if (tcgetattr(fd, &currenttio) == -1)
		return false;
	if (baudrate)
		*baudrate = cfgetispeed(&currenttio);
	if (databits)
	{
		switch (currenttio.c_cflag & CSIZE)
		{
			case CS8: *databits = 8; break;
			case CS7: *databits = 7; break;
			case CS6: *databits = 6; break;
			case CS5: *databits = 5; break;
			default: settingsvalid = false; break;
		}
	}
	if (parity)
	{
		if (!(currenttio.c_cflag & PARENB))
			*parity = 'N';
		else if (currenttio.c_cflag & PARODD)
			*parity = 'O';
		else
			*parity = 'E';
	}
	if (stopbits)
		*stopbits = (currenttio.c_cflag & CSTOPB) ? 2 : 1;
...
bool serialCOM::SetPortSettings(double baudrate, unsigned short databits, char parity, float stopbits)
{
	...
	if (!settingsvalid)
		return false;
	return (tcsetattr(fd, TCSANOW, &newtio) != -1);
#else
	int cIOCTL = -1;
	int cIOCTL_SD = -1;
	bool isStdBaud = true;
...
bool serialCOM::OpenPort(std::string portname)
{
	...
	confirm = ioctl(fd, TIOCEXCL, NULL);
	confirm = fcntl(fd, F_SETFL, FNDELAY);
	if (confirm == -1)
	{
		ClosePort();
		return false;
	}
	settingssaved = (tcgetattr(fd, &oldtio) != -1);
	ioctl_tiocgserial_supported = false;
	ioctl_tiocsserial_supported = false;
	flag_async_low_latency_supported = false;
	breakset = false;
...
bool serialCOM::SendBreak(unsigned int duration_ms)
{
	...
	{
#ifdef __APPLE__
		confirmSB = tcsendbreak(fd, duration_ms / 100);
#else
