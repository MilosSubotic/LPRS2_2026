...
QString CUcontent_MBsSWs::autoMBSWselectionFileName() const
{
	if (!_SSMPdev)
		return "";
	QString ROM_ID = QString::fromStdString(_SSMPdev->getROMID());
	if (ROM_ID.isEmpty())
		return "";
	for (int i=0; i<ROM_ID.size(); i++)
	{
		const QChar c = ROM_ID.at(i);
		if (!c.isLetterOrNumber() && c != '_' && c != '.' && c != '-')
			ROM_ID[i] = '_';
	}
	QDir dir(QDir::homePath() + "/FreeSSM/profiles");
	if (!dir.exists())
		QDir().mkpath(dir.absolutePath());
	return dir.absoluteFilePath("mbsw_" + ROM_ID + ".list");
}


void CUcontent_MBsSWs::saveAutoMBSWselection()
{
	if (!_SSMPdev || _MBSWreading)
		return;
	QString filename = autoMBSWselectionFileName();
	if (!filename.isEmpty())
		saveMBsSWs(filename);
}


void CUcontent_MBsSWs::loadAutoMBSWselection()
{
	QString filename = autoMBSWselectionFileName();
...
QString CUcontent_MBsSWs::logDirectoryName() const
{
	QDir dir(QDir::homePath() + "/FreeSSM/logs");
	if (!dir.exists())
		QDir().mkpath(dir.absolutePath());
...
bool CUcontent_MBsSWs::openMBSWlog()
{
	closeMBSWlog();
	QString ROM_ID = _SSMPdev ? QString::fromStdString(_SSMPdev->getROMID()) : "unknown";
	for (int i=0; i<ROM_ID.size(); i++)
	{
		const QChar c = ROM_ID.at(i);
		if (!c.isLetterOrNumber() && c != '_' && c != '.' && c != '-')
	...
	_logFileName = logDirectoryName() + QDir::separator() +
	               "mbsw_" + ROM_ID + "_" + QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss") + ".csv";
	_logFile.open(_logFileName.toLocal8Bit(), std::ios::out | std::ios::trunc);
	_logHeaderWritten = false;
	if (!_logFile.is_open())
	{
		errorMsg(tr("Log Error"), tr("Error: failed to create the MB/SW log file."));
		return false;
	}
	return true;
}
...
void CUcontent_MBsSWs::closeMBSWlog()
{
	if (_logFile.is_open())
		_logFile.close();
	_logHeaderWritten = false;
...
QString CUcontent_MBsSWs::csvEscape(QString value)
{
	value.replace("\"", "\"\"");
	return "\"" + value + "\"";
}
...
void CUcontent_MBsSWs::writeMBSWlogRow(const std::vector<unsigned int>& rawValues, const std::vector<QString>& valueStrList,
                                       const std::vector<QString>& unitStrList, int refreshduration_ms)
{
	if (!_logFile.is_open())
		return;
	if (!_logHeaderWritten)
	{
		_logFile << "timestamp,refresh_ms,type,title,value,raw,unit\n";
	...
	for (size_t k=0; k<_MBSWmetaList.size(); k++)
	{
		...
		_logFile << csvEscape(timestamp).toUtf8().constData() << ","
		         << refreshduration_ms << ","
		         << csvEscape(type).toUtf8().constData() << ","
		         << csvEscape(title).toUtf8().constData() << ","
		         << csvEscape(valueStrList.at(tablePosIndex)).toUtf8().constData() << ","
		         << rawValues.at(k) << ","
		         << csvEscape(unitStrList.at(tablePosIndex)).toUtf8().constData() << "\n";
	}
	_logFile.flush();
}
...
void CUcontent_MBsSWs::processMBSWRawValues(const std::vector<unsigned int>& rawValues, int refreshduration_ms)
{
	...
	writeMBSWlogRow(rawValues, valueStrList, unitStrList, refreshduration_ms);
