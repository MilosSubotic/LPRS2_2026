...
IMPREZA®, FORESTER®, EXIGA® and TRIBECA® starting with model year 1999. The
main supported control units are engine and transmission; this development
version also contains basic SSM1 support for ABS/VDC, Cruise Control and Air
Conditioning control units where matching definitions are available.
...
0. TABLE OF CONTENTS:
   ...
   5. Installation
   6. Starting FreeSSM
   7. Current Development Features
...
2. SUPPORTED PLATFORMS:
   ...
   - macOS / Unix-like systems with POSIX serial support

The current project includes macOS build and serial-port compatibility changes.
Runtime behaviour still depends on the selected diagnostic interface and driver.
...
3.) MinGW (only for MS Windows)
   ...
   4.) Diagnostic interface
       FreeSSM can use three interface classes:
        - serial pass-through K/L or KK/L interfaces
        - SAE J2534 pass-through interfaces
        - AT-command controlled interfaces, including supported ELM/AGV/Diamex
          compatible devices
       Availability of protocols depends on the selected hardware, driver and
       platform.
...
This project version differs from the original 1.x documentation in several
user-visible areas:

   - The control-unit dialog contains a Connect button. If the diagnostic
     interface opens but the ECU does not answer yet, the selected screen stays
     open and the user can switch ignition on and press Connect again.
   - Engine and transmission dialogs still open Diagnostic Codes by default.
     Use the Measuring Blocks button or command line option "-f mbssws" to start
     directly in Measuring Blocks/Switches mode.
   - Measuring Blocks/Switches selections are automatically saved per ECU ROM ID
     in:
        ~/FreeSSM/profiles/mbsw_<ROM_ID>.list
     The .list file is FreeSSM's internal binary selection format. It stores
     the ECU ROM ID together with MB/SW metadata and is validated against the
     current ECU before loading. It is not a CSV/text log.
   - When Measuring Blocks/Switches reading starts, FreeSSM writes a CSV log in:
        ~/FreeSSM/logs/mbsw_<ROM_ID>_<yyyyMMdd_HHmmss>.csv
     The CSV columns are:
        timestamp,refresh_ms,type,title,value,raw,unit
   - If a communication error happens, the dialog keeps the last selected mode
     visible where possible and lets the user reconnect manually. This version
     does not implement automatic timer-based reconnect attempts.
