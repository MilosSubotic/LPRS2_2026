...
ControlUnitDialog::ControlUnitDialog(QString title, AbstractDiagInterface *diagInterface, QString language, bool preferSSM2protocolVariantISO14230)
{
	...
	_infoWidget = NULL;
	_contentWidget = NULL;
	_connect_pushButton = NULL;
	_setup_done = false;
	_connected = false;
	_handlingCommunicationLoss = false;
	_communicationErrorActive = false;
	_mode = Mode::None;
	_startupContentSelection = ContentSelection::DCsMode;
	_startupAutostart = false;
	_content_DCs = NULL;
	...
	_connect_pushButton = new QPushButton(selection_groupBox);
	_connect_pushButton->setText(tr("Connect"));
	_connect_pushButton->setIcon(QIcon(QString::fromUtf8(":/icons/chrystal/22x22/hardware.png")));
	_connect_pushButton->setIconSize(QSize(22, 22));
#ifndef SMALL_RESOLUTION
	_connect_pushButton->setFixedWidth(160);
	_connect_pushButton->setFixedHeight(35);
#else
	_connect_pushButton->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
#endif
	selButtons_verticalLayout->addWidget(_connect_pushButton);
	connect(_connect_pushButton, SIGNAL(released()), this, SLOT(connectToControlUnit()));
}
...
ControlUnitDialog::~ControlUnitDialog()
{
	...
	if (_connect_pushButton)
		disconnect(_connect_pushButton, SIGNAL(released()), this, SLOT(connectToControlUnit()));
	if (_infoWidget)
		delete _infoWidget;
...
bool ControlUnitDialog::setup(ContentSelection csel, QStringList cmdline_args)
{
	...
	if (!getParametersFromCmdLine(&cmdline_args, &mbssws_selfile, &autostart))
		exit(ERROR_BADCMDLINEARGS);
	_startupContentSelection = csel;
	_startupMBSWselectionFile = mbssws_selfile;
	_startupAutostart = autostart;
	// ***** Create and insert the content widget *****:
	if (!prepareContentWidget(mode))
		return false;
	_setup_done = true;
	// ***** Connect to Control Unit *****:
	initializeControlUnit(mode, true);
	return true;
}


bool ControlUnitDialog::initializeControlUnit(Mode mode, bool showSuccessMessage)
{
	bool supported = false;
	SSMprotocol::CUsetupResult_dt init_result;
	...
	init_result = probeProtocol( controlUnitType() );
	...
	initstatusmsgbox.close();
	_connected = true;
	_connect_pushButton->setEnabled(false);
	_connect_pushButton->setText(tr("Connected"));
	// Run requested startup functionality after the ECU is ready:
	if (_startupContentSelection == ContentSelection::ClearMemoryFcn)
		clearMemory();
	else if (_startupContentSelection == ContentSelection::ClearMemory2Fcn)
		clearMemory2();
	else
		applyMBSWStartupParameters(_startupContentSelection);
	return true;
...
commError:
	...
	if (_diagInterface->isConnected())
		_diagInterface->disconnect();
	setDisconnectedState(tr("Control Unit is not connected yet. Switch ignition on, then press Connect."));
	return false;
}
...
void ControlUnitDialog::applyMBSWStartupParameters(ContentSelection csel)
{
	if ((csel != ContentSelection::MBsSWsMode) || (_content_MBsSWs == NULL))
		return;
	if (_startupMBSWselectionFile.size())
		_content_MBsSWs->loadMBsSWs(_startupMBSWselectionFile);
	if (_content_MBsSWs->numMBsSWsSelected() && _startupAutostart)
		_content_MBsSWs->startMBSWreading();
}


void ControlUnitDialog::setDisconnectedState(QString message)
{
	_connected = false;
	if (_connect_pushButton)
	{
		_connect_pushButton->setEnabled(true);
		_connect_pushButton->setText(tr("Connect"));
	}
	QList<ContentSelection> selections = _contentSelectionButtons.keys();
	for (int k=0; k<selections.size(); k++)
		setContentSelectionButtonEnabled(selections.at(k), false);
#ifndef SMALL_RESOLUTION
	if (_ifstatusbar)
	{
		_ifstatusbar->setProtocolName(message, Qt::darkYellow);
		_ifstatusbar->setBaudRate("---");
	}
#endif
}
...
void ControlUnitDialog::connectToControlUnit()
{
	Mode mode;
	if (_connected)
		return;
	mode = _mode;
	if ((mode == Mode::None) && !getModeForContentSelection(_startupContentSelection, &mode))
		mode = Mode::DCs;
	if (initializeControlUnit(mode, true))
		return;
	QMessageBox msg(QMessageBox::Information, tr("Not connected"),
	                tr("The Control Unit did not answer yet.\nSwitch ignition on and try again."),
	                QMessageBox::Ok, this);
	QFont msgfont = msg.font();
	msgfont.setPointSize(9);
	msg.setFont(msgfont);
...
bool ControlUnitDialog::startDCsMode()
{
	...
	if (DCgroups != SSMprotocol::noDCs_DCgroup)
	{
		if (!_content_DCs->startDCreading())
			return false;
		connect(_content_DCs, SIGNAL( error() ), this, SLOT( connectionLost() ) );
	}
	_mode = Mode::DCs;
	return true;
}
...
bool ControlUnitDialog::startMBsSWsMode()
{
	...
	if (!_content_MBsSWs->setup(_SSMPdev))
		return false;
	if (!_lastMBSWmetaList.empty())
	{
		if (!_content_MBsSWs->setMBSWselection(_lastMBSWmetaList))
			return false;
	}
	connect(_content_MBsSWs, SIGNAL( error() ), this, SLOT( connectionLost() ) );
	_mode = Mode::MBsSWs;
	return true;
}
...
bool ControlUnitDialog::startAdjustmentsMode()
{
	...
	if (!_content_Adjustments->setup(_SSMPdev))
		return false;
	connect(_content_Adjustments, SIGNAL( communicationError() ), this, SLOT( connectionLost() ) );
...
bool ControlUnitDialog::startSystemOperationTestsMode()
{
	...
	if (!_content_SysTests->setup(_SSMPdev))
		return false;
	connect(_content_SysTests, SIGNAL( error() ), this, SLOT( connectionLost() ) );
	_mode = Mode::SysTests;
	return true;
}
...
void ControlUnitDialog::handleCommunicationLoss()
{
	Mode mode = _mode;

	if (_handlingCommunicationLoss)
		return;
	_handlingCommunicationLoss = true;
	saveContentSettings();
	if (_SSMPdev)
	{
		disconnect( _SSMPdev, SIGNAL( commError() ), this, SLOT( communicationError() ) );
	...
	if (_diagInterface->isConnected())
		_diagInterface->disconnect();
	_connected = false;
	setDisconnectedState(tr("Connection lost. Switch ignition on, check the diagnostic interface, then press Connect."));
	if (mode != Mode::None)
	{
...
void ControlUnitDialog::connectionLost()
{
	handleCommunicationLoss();
}
...
void ControlUnitDialog::communicationError(QString addstr)
{
	...
	_communicationErrorActive = true;
