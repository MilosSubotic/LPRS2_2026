...
volatile u8 current_row_grp = 0;

void refresh_row_group(u8 row_grp){


	// select HUB75 address
	screen.addr = row_grp;
	...
	for(int row_shift = 2; row_shift >= 0; row_shift--){
		...
		for(int col = 0; col < 64; col++){
			...
			for(int screen_half = 0; screen_half < 2; screen_half++){
				...
				int row = screen_half * 16
						+ row_shift * 8
						+ row_grp;

				bool pix = bitchase_text(row, col);
...
{
	// disable LEDs while shifting
	screen.n_oe = 1;

	refresh_row_group(current_row_grp);
